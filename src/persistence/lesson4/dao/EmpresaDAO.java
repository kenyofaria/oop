...
public class EmpresaDAO {
	...
	public List<Empresa> listaAscByNome(){
		List<Empresa> listaDeEmpresas = new ArrayList<Empresa>();
		conexao = BancoDeDados.getConexao();
		try {
			PreparedStatement ps = conexao.prepareStatement("select * from empresas order by nome");
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Empresa empresa = new Empresa(rs.getString(1), rs.getString(2));
	...
	public List<Empresa> listaDescByNome(){
		List<Empresa> listaDeEmpresas = new ArrayList<Empresa>();
		conexao = BancoDeDados.getConexao();
		try {
			PreparedStatement ps = conexao.prepareStatement("select * from empresas order by nome desc");
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Empresa empresa = new Empresa(rs.getString(1), rs.getString(2));
				listaDeEmpresas.add(empresa);
	...
	public List<Empresa> listaAscByCnpj(){
		List<Empresa> listaDeEmpresas = new ArrayList<Empresa>();
		conexao = BancoDeDados.getConexao();
		try {
			PreparedStatement ps = conexao.prepareStatement("select * from empresas order by cnpj");
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				Empresa empresa = new Empresa(rs.getString(1), rs.getString(2));
				listaDeEmpresas.add(empresa);
